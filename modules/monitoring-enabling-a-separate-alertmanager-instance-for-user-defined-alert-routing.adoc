...
* For {product-rosa-hcp} and {product-dedicated}:
...
* For {ocp}: verify that the `user-workload` Alertmanager instance has started:
+
[source,terminal]
----
...
* For {product-rosa-hcp} and {product-dedicated}: verify that the `alert-manager-user-workload` pods are running:
+
[source,terminal]
----
